...
class Confetti extends Component<ConfettiProps> {
  ...
  render() {
    ...
    return (
      <canvas
        height={String(height * PIXEL_RATIO)}
        width={String(width * PIXEL_RATIO)}
        onClick={ev => this.start(ev.clientX, ev.clientY)}
        style={{ width, height }}
        // TODO(mdr): We found a new Flow error when upgrading:
        //     "(node: HTMLCanvasElement) => (this.canvas = node) (function) This type is incompatible with canvas (union: function type | string)"
        //     $FlowFixMe(0.52.0->0.53.0)
